...
// 首次呼叫會下載模型權重，之後離線可用。跟 clipScorer.ts 用同一顆 model
// （Xenova/clip-vit-base-patch32），且必須跟前端 asterism repo 的
// clipEmbedding.service.ts 用同一種抽取方式（image-feature-extraction + 手動
// L2 normalize），向量空間才會一致。
export async function createClipEmbedder(model = 'Xenova/clip-vit-base-patch32'): Promise<Embedder> {
